...
public class IndexController extends BasePluginController {
	...
	public OrderStatisticsVO statistics(HttpServletRequest request,Model model,
		...
		//查询的sql
		String sql = "SELECT COUNT(id) AS orderNumber,SUM(pay_money) AS money,state FROM shop_order WHERE storeid = "+store.getId()+" AND addtime >"+startDay+" AND addtime < "+endDay+"  GROUP BY shop_order.state ";
