...
public class Goods extends BaseEntity implements java.io.Serializable{
	...
	private Short putaway;		//是否上架在售，<ul><li>1:出售中<li>0已下架</ul>
	...
	private Integer userBuyRestrict;	//用户购买限制。<ul><li>如果值是0，则可以任意购买，没有什么限制<li>如果是1，则代表每个用户只能购买一个<li>如果是2，代表每个用户只能购买2个以内，不超过2个</ul>只要是下单了，未退单成功的，都算是购买了
	 ...
	 */
	private String specification;	//商品规格，型号 v1.6增加。比如商品是衣服，这里就可以是 黑色、黄色 等颜色的。里面的数据格式为JSONArray类型。 存入的格式为： [{"黄色":90.1},{"黑色":80},{"白色":70.5}]  其中 key是对用户显示的规格文字， value是这个规格的价格，float类型
