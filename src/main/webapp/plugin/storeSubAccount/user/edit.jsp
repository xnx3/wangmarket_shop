...
function authorityTwoReplace(item){
	return authorityTemplate
		...
		.replace(/\{subInput\}/g,item.isUse == 1 ? '<input type=checkbox name=menu value='+ item.id + ' title='+ item.name +' checked '+' >': '<input type=checkbox name=menu value='+ item.id + ' title='+ item.name +'>' )
		;
}
//列表的模版
var subList = '';
...
function  authorityOneReplace(item){
	...
	return authorityOneTemplate
		...
		.replace(/\{input\}/g,item.isUse == 1 ? '<input type=checkbox name=menu value='+ item.id + ' title='+ item.name +' checked '+' >': '<input type=checkbox name=menu value='+ item.id + ' title='+ item.name +'>' )
		;
}
