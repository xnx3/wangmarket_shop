...
<div>
	...
	<div id="Ali" style="display: block;">
		...
		<botton class="layui-btn layui-btn-xs layui-btn-primary" onclick="addOrUpdate('alipayAppPrivateKey','${paySet.alipayAppPrivateKey}','APP应用的私钥编辑')"  style="margin-left: 3px;margin-top:-1px;">编辑</botton>
		 ...
		 <a class="layui-btn layui-btn-xs aa" lay-data = "{url: '/shop/storeadmin/paySet/uploadCrt.do?name=alipayCertPublicKeyRSA2'}" style="margin-top: 3px;">编辑</a>	 
		
		</br>支付宝的根证书:&nbsp;&nbsp;${paySet.alipayRootCert }
		 <a class="layui-btn layui-btn-xs aa" lay-data = "{url: '/shop/storeadmin/paySet/uploadCrt.do?name=alipayRootCert'}" style="margin-top: 3px;">编辑</a>	 
		
		</br>支付宝的应用公钥证书:&nbsp;&nbsp;${paySet.alipayAppCertPublicKey }
		 <a class="layui-btn layui-btn-xs aa" lay-data = "{url: '/shop/storeadmin/paySet/uploadCrt.do?name=alipayAppCertPublicKey2'}" style="margin-top: 3px;">编辑</a>	 
...
function updateState(name1,value1) {
	...
	$.post('/shop/storeadmin/paySet/setUse.do?name=' + name1 + '&value=' + value1 , function(data){
...
function addOrUpdate(name1,value1,text){
	layer.prompt({
		  ...
		  $.post('/shop/storeadmin/paySet/setUse.do?name=' + name1 + '&value=' + value , function(data){
			    parent.iw.loadClose();    //关闭“操作中”的等待提示
			    if(data.result == '1'){
			        parent.iw.msgSuccess('操作成功');
			        window.location.reload();	//刷新当前页
