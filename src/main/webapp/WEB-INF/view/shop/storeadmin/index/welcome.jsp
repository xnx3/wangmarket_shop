...
<table class="layui-table iw_table" lay-even lay-skin="nob" style="margin:3%; width:94%;">
	<tbody>
		...
		<tr>
			...
			<td>
				 <a class="layui-btn layui-btn-sm uploadImg" lay-data = "{url: '/shop/storeadmin/index/uploadImg.do?storeId=${store.id}'}" style="\"><i class="layui-icon layui-icon-upload"></i></a>
			</td>
		</tr>
		<tr>
...
function toEditPage(id,type) {
	...
	content: '/shop/storeadmin/index/toEditPage.do?storeId=' + id + '&type=' + type
	});	  
}
...
function addOrUpdate(id,value,name,text){
	layer.prompt({
		  ...
		  $.post('/shop/storeadmin/index/save.do?storeId='+id +'&'+name+'=' + value, function(data){
...
function stateUpdate(id) {
	...
	layer.confirm('修改店铺状态', {
		  btn: ['营业中','已打烊'] //按钮
			...
			$.post('/shop/storeadmin/index/save.do?storeId='+id +'&state=1', function(data){
			    parent.iw.loadClose();    //关闭“操作中”的等待提示
			    if(data.result == '1'){
			        parent.iw.msgSuccess('操作成功');
			        window.location.reload();	//刷新当前页
			...
			$.post('/shop/storeadmin/index/save.do?storeId='+id +'&state=2', function(data){
			    parent.iw.loadClose();    //关闭“操作中”的等待提示
			    if(data.result == '1'){
			        parent.iw.msgSuccess('操作成功');
			        window.location.reload();	//刷新当前页
	...
	,done: function(res){
		...
		if(res.result == '1'){
			parent.iw.msgSuccess("上传成功");
			 window.location.href = '/shop/storeadmin/index/welcome.do';
