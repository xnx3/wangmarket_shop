...
<div>
	...
	当前已
  	<c:if test="${smsSet.useSms == 0}">关闭
  		 <botton class="layui-btn layui-btn-xs layui-btn-primary" onclick="update('useSms','1');" style="margin-left: 3px;margin-top:-1px;">开启</botton>	
  	</c:if>
  	<c:if test="${smsSet.useSms == 1}">开启
...
<c:if test="${smsSet.useSms == 1}">
	...
	</br>密码:&nbsp;&nbsp;<x:substring maxLength="4" text="${smsSet.password }" more="****"></x:substring>
	...
	<script>
		$.post('/shop/store/sms/getBalance.do',function(data){
			var info = data.info;
			if(data.result == 1){
				//成功获取到了条数
	...
	某个ip一天最多能发多少条短信:&nbsp;&nbsp;${smsSet.quotaDayIp}
	...
	</br>某个手机号一天最多能发多少条短信:&nbsp;&nbsp;${smsSet.quotaDayPhone}
...
function update(name,value) {
	...
	$.post('/shop/store/sms/update.do', data,function(data){
